...
- **Basic Information / 基本信息:** Support Triage workspace 的 `Sort: Timestamp`；Product Hardening PH-010；2026-08-14；状态 `VERIFIED`。
...
- **Manual Observation / 人工观察:** PH-010 read-only audit 追踪了全部 timestamp 的产生、保存、展示、导出与排序路径。system-generated audit timestamp 已由 contracts 层 `require_utc` 强制 UTC，无缺陷；用户记录 timestamp 经 Batch CSV `_parse_timestamp` 进入，允许 naive 与任意 offset，其语义（是否声明 offset、naive 与否）被保留，但不保证逐字保留原始拼写——`_parse_timestamp` 产出的是解析后的 `datetime`，`Z` 会在其上再序列化为 `+00:00`，date-only 值会带上午夜时间。解析后的 CSV field value（经 `csv.DictReader` 处理，已去除原始 quoting/escaping 语法，不是文件原始 bytes）另由 `PreparedBatchRow.input_values` 单独保留；Batch export 使用该 field value 而非解析后的 datetime 表示，且仍会经过既有 `safe_spreadsheet_text()` 转义并由 `csv.DictWriter` 重新序列化，因此不构成对原始 CSV 源语法的 byte-for-byte 保留保证；Triage `source_timestamp`（排序所读的值）来自解析后 `datetime` 的 `.isoformat()`，因此可能与用户原始拼写不同，但排序本身不会进一步改写它。字典序因此不等价于真实时刻顺序：`2026-07-01T08:00:00-05:00`（13:00Z）的字符串排在 `2026-07-01T10:00:00+00:00`（10:00Z）之前，顺序被静默颠倒。该分支此前无任何测试覆盖。
...
- **Git / PR Record / Git 与 PR 记录:** `hardening/a9-timestamp-ordering`；baseline main SHA `7ef4d29d2bb98af2a60003748a575de8c5bcda96`；behavioral candidate `076fbe4f883073a1961b024d52d06d9cac22a58a`；PR #27 reviewed head `ef9b54adc3eeed98ef4154f71ae7f69e5f6000cb`；formal review PASS；final-head Python 3.11/3.12/3.13 CI PASS。
- **Final Outcome / 最终结果:** 9 个定向回归通过，其中 5 个在修复前的实现上确实失败，证明覆盖真实缺陷；完整 suite 181 passed / 2 opt-in skipped，Ruff、strict MyPy(74 files)、compileall、pip check 全部通过。真实浏览器 smoke 使用含两个不同 offset、一个 naive 与一个 missing timestamp 的 synthetic batch，确认顺序为 10:00Z → 13:00Z → naive 1999 → missing，且排序前后 Triage 存储值本身未被排序操作改写。FCR-051 在固定 behavioral SHA 上关闭为 `VERIFIED`；PH-010 已关闭；Feature Freeze 保持 PASS，FCR-042/043 保持 `OPEN` non-blocking。
...
- **Basic Information:** The Support Triage workspace `Sort: Timestamp` control; Product Hardening PH-010; 2026-08-14; status `VERIFIED`.
...
- **Manual Observation:** The PH-010 read-only audit traced every timestamp creation, storage, display, export, and ordering path. System-generated audit timestamps are already forced to timezone-aware UTC by `require_utc` in the contracts layer and showed no defect. User record timestamps enter through the Batch CSV `_parse_timestamp`, which permits naive values and any offset and preserves their semantics — whether an offset is stated and whether the value is naive — but not necessarily their exact spelling: `_parse_timestamp` produces a parsed `datetime`, so a `Z` offset is re-rendered as `+00:00` and a date-only value gains an explicit midnight time. The parsed CSV field value (produced by `csv.DictReader`, with the original quoting/escaping syntax already resolved, not the file's raw bytes) is retained separately in `PreparedBatchRow.input_values`. Batch export uses that field value rather than the parsed datetime representation, and still applies the existing spreadsheet-safety escaping and normal `csv.DictWriter` serialization, so this is not a byte-for-byte preservation guarantee of the original CSV source syntax. The Triage `source_timestamp` that sorting reads is `record.timestamp.isoformat()` on the parsed value, so it can already differ textually from what the user typed, though the sort itself never rewrites it further. Lexicographic order therefore does not equal real-instant order: `2026-07-01T08:00:00-05:00` (13:00Z) sorts as text before `2026-07-01T10:00:00+00:00` (10:00Z), silently reversing them. The branch had no test coverage at all.
...
- **Git / PR Record:** `hardening/a9-timestamp-ordering`; baseline main SHA `7ef4d29d2bb98af2a60003748a575de8c5bcda96`; behavioral candidate `076fbe4f883073a1961b024d52d06d9cac22a58a`; PR #27 reviewed head `ef9b54adc3eeed98ef4154f71ae7f69e5f6000cb`; formal review PASS; final-head Python 3.11/3.12/3.13 CI PASS.
- **Final Outcome:** Nine targeted regressions passed, and five of them genuinely fail against the pre-fix implementation, confirming they cover the real defect. The full suite passed 181 tests with 2 opt-in model integrations skipped, and Ruff, strict MyPy for 74 files, compileall, and pip check passed. A real-browser smoke using a synthetic batch with two different offsets, one naive timestamp, and one missing timestamp confirmed the order 10:00Z, 13:00Z, naive 1999, missing, and confirmed that the sort operation itself did not rewrite the stored Triage values. FCR-051 closes as `VERIFIED` on the fixed behavioral SHA; PH-010 is closed. Feature Freeze remains PASS, and FCR-042/043 remain `OPEN` non-blocking.
